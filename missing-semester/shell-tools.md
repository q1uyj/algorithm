...
> 需要使用 `source mcd.sh`执行脚本并把它加载到 shell 环境中。之后就可以使用 `mcd  test` 来创建并进入文件夹了。
...
```bash
missing-semester % foo=$(pwd)

missing-semester % echo foo
foo

missing-semester % echo $foo
/Users/qiuyijun/work/algorithm/missing-semester

missing-semester % echo $pwd
...
algorithm % echo "We are in $(pwd)"
We are in /Users/qiuyijun/work/algorithm

```

当您通过 `$( CMD )` 这样的方式来执行`CMD` 这个命令时，它的输出结果会替换掉 `$( CMD )` 。例如，如果执行 `for file in $(ls)` ，shell首先将调用`ls` ，然后遍历得到的这些返回值。还有一个冷门的类似特性是 *进程替换*（*process substitution*）， `<( CMD )` 会执行 `CMD` 并将结果输出到一个临时文件中，并将 `<( CMD )` 替换成临时文件名。这在我们希望返回值通过文件而不是STDIN传递时很有用。例如，`cat <(cmd1) <(cmd2)`会将两个结果合在一起输出， `diff <(ls foo) <(ls bar)` 会显示文件夹 `foo` 和 `bar` 中文件的区别。

> `diff`命令简介：
>
> < 对应第一个文件，> 对应第二个文件
>
> `1,3c1,9` /  `6c6` 分别代表 第一个文件的 1~3 行和第二个文件的 1~9 行不同；两个文件的第 6 行不同
...
for file in "$@"; do
    ...
    # 我们将标准输出流和标准错误流重定向到Null，因为我们并不关心这些信息（如果有foobar，grep输出的语句）
    if [[ $? -ne 0 ]]; then
...
2. 编写两个bash函数 `marco` 和 `polo` 执行下面的操作。 每当你执行 `marco` 时，当前的工作目录应当以某种形式保存，当执行 `polo` 时，无论现在处在什么目录下，都应当 `cd` 回到当时执行 `marco` 的目录。 为了方便debug，你可以把代码写在单独的文件 `marco.sh` 中，并通过 `source marco.sh`命令，（重新）加载函数。
    ...
    ```bash
    # result:
    qiuyijun@qiuyijundeMacBook-Air missing-semester % marco
    save pwd /Users/qiuyijun/work/algorithm/missing-semester
    qiuyijun@qiuyijundeMacBook-Air missing-semester % cd /
    qiuyijun@qiuyijundeMacBook-Air / % polo
    qiuyijun@qiuyijundeMacBook-Air missing-semester % 
    ```
